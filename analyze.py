...
def get_mean(im, model, device, batch=False):
    model.eval()
    if len(im.shape) == 3:
        im = torch.unsqueeze(im, dim=0).to(device)
    else:
...
if __name__ == "__main__":
    ...
    IT = int(TOTAL / BATCH)
    # IT=5
    embeddings = torch.empty(0, 100)
    if not os.path.exists(".\\embeddings.npy"):
        for i in range(IT + 1):
            #print(str(i) + " / " + str(IT))
            if (i + 1) * BATCH > len(files):
                bat = files[i * BATCH:]
            else:
                bat = files[i * BATCH:(i + 1) * BATCH]
            imgs = torch.stack(prep.get_ims(bat))
            z = get_mean(imgs, model, "cpu")
    ...
    pca.fit(embeddings)
    np.save(".\\fullPCA.npy", pca.components_)
